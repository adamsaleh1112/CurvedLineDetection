...
while (True):
    ...
    eroded = cv2.erode(blur, kernel, iterations=1)  # eroding frame, which removes one pixel around the white areas, thus thinning the lines which have been turned white with thresholding
    ...
    # DETECT LINES
    lines = cv2.HoughLinesP(maskimg, rho=1, theta=np.pi / 180, threshold=10, minLineLength=3, maxLineGap=5)  # detecting lines with given parameters
    ...
    if lines is not None:  # if lines are detected
        ...
        for line in lines:  # iterates through each line detected
            ...
            cv2.line(img, (x1, y1), (x2, y2), (255, 0, 0),
                     10)  # draws line by plotting two points (x1,y1) (x2,y2) and draws a line between them
